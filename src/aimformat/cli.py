...
def _cmd_note(args: argparse.Namespace) -> int:
    if args.check and args.remove:
        print("aim: --check and --remove are mutually exclusive",
              file=sys.stderr)
        return 2
    missing = [p for p in args.files if not Path(p).is_file()]
    if missing:
    ...
    for path in args.files:
        ...
        if args.remove:
            ...
            status = ("ok" if doc.has_canonical_note()
                      else "stale" if doc.note is not None else "missing")
            failures += status != "ok"
...
def _cmd_propose(args: argparse.Namespace) -> int:
    ...
    if args.action == "modify":
        p = doc.propose_modify(args.target, markup, author=author,
                               explanation=args.explanation)
    elif args.action == "add":
        p = doc.propose_add(markup, author=author, container=args.container,
                            after=after, explanation=args.explanation)
    elif args.action == "delete":
        p = doc.propose_delete(args.target, author=author,
                               explanation=args.explanation)
    elif args.action == "move":
        p = doc.propose_move(args.target, author=author,
                             container=args.container, after=after,
                             explanation=args.explanation)
    else:  # theme
        slots: dict[str, str] = {}
        for item in args.set:
            if "=" not in item:
                print(f"aim: --set expects SLOT=VALUE, got {item!r}",
                      file=sys.stderr)
                return 2
            k, v = item.split("=", 1)
    ...
    doc.save(out)
    if args.format == "json":
        print(json.dumps({"proposal": p.id, "action": p.action,
                          "target": p.target,
                          "author": _actor_str(p.author),
                          "explanation": p.explanation,
                          "file": str(out)}, indent=2))
    else:
        print(p.id)
        print(f"wrote {out}")
    return 0
...
def _cmd_resolve(args: argparse.Namespace, decision: str) -> int:
    ...
    if not args.all and not args.pids:
        print("aim: nothing to do (give proposal ids or --all)",
              file=sys.stderr)
        return 2
    doc = AimDocument.load(args.file)
    ...
    for pid in pids:
        if decision == "accept":
            events.append(doc.accept(pid, decided_by=decided_by,
                                     explanation=args.explanation))
        else:
            events.append(doc.reject(pid, decided_by=decided_by,
                                     explanation=args.explanation))
    out = Path(args.output or args.file)
    doc.save(out)
    if args.format == "json":
        print(json.dumps([{"proposal": pid, "decision": decision,
                           "seq": ev.seq}
                          for pid, ev in zip(pids, events)], indent=2))
    else:
        for pid in pids:
...
def _cmd_mcp(args: argparse.Namespace) -> int:
    try:
        ...
        print("aim: MCP support requires the optional extra: "
              "pip install 'aimformat[mcp]'", file=sys.stderr)
        return 2
...
def _cmd_show(args: argparse.Namespace) -> int:
    ...
    if args.format == "json":
        ...
        print(json.dumps({
            "file": str(args.file), "title": doc.title, "lang": doc.lang,
            "spec_version": doc.spec_version, "seq": doc.seq,
            "doc_hash": doc.doc_hash, "summary_stale": summary_stale,
            ...
            "history_events": len(doc.history)}, indent=2))
        return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p = sub.add_parser("note",
                       help="add or refresh the agent note (spec §2.5); "
                            "modifies files in place")
    p.add_argument("files", nargs="+")
    p.add_argument("--check", action="store_true",
                   help="verify without writing; exit 1 when any file "
                        "lacks a canonical note")
    p.add_argument("--remove", action="store_true",
                   help="strip the note instead of adding it")
    p.add_argument("--format", choices=["text", "json"], default="text")
    ...
    def _proposal_common(pp: argparse.ArgumentParser) -> None:
        pp.add_argument("--author", default="external:aim-cli",
                        help="human:ID | agent:MODEL | external:ID "
                             "(default: external:aim-cli)")
        pp.add_argument("--explanation",
                        help="one-line why; raw-tier readers see only this")
    ...
    pa.add_argument("--after",
                    help="anchor id ('first' = first position; "
                         "default: end of container)")
    _proposal_common(pa)
    ...
    for verb, fn in (("accept", _cmd_accept), ("reject", _cmd_reject)):
        p = sub.add_parser(verb,
                           help=f"{verb} pending proposals; modifies the "
                                "file in place unless -o is given")
        p.add_argument("file")
        p.add_argument("pids", nargs="*", metavar="PID")
        p.add_argument("--all", action="store_true",
                       help=f"{verb} every pending proposal")
        p.add_argument("--author", default="external:aim-cli",
                       help="human:ID | agent:MODEL | external:ID")
