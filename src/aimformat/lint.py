...
class _Linter:
    ...
    def structure(self) -> None:
        ...
        metas = [
            ...
            if e.tag == "script" and e.get("type") == REGISTRY.script_types["meta"]
        ]
        if len(metas) > 1:
        ...
        notes = [
            ...
            if isinstance(n, Comment) and n.data.lstrip().startswith("aim-note:")
        ]
        if len(notes) > 1:
            self.add("S030", WARNING, "more than one aim-note comment in <head>")
        for node in self.state.body.children:
            if isinstance(node, Comment):
                self.add(
                    "S007",
                    ERROR,
                    "comments are not allowed in <body> (canonical form keeps them in <head> only)",
                )
            elif isinstance(node, Text) and node.data.strip():
                self.add("S008", ERROR, f"stray text in <body>: {node.data.strip()[:40]!r}")
    ...
    def document_shell(self) -> None:
            ...
            if isinstance(node, Element):
                if node.tag == "html":
                    ...
                    self.add(
                        "S028",
                        ERROR,
                        f"unexpected top-level <{node.tag}> outside the <html> document element",
                    )
            elif isinstance(node, Comment):
                self.add("S028", ERROR, "comment outside the <html> document element")
            elif isinstance(node, Text) and node.data.strip():
                self.add(
                    "S028",
                    ERROR,
                    f"stray text outside the <html> document element: {node.data.strip()[:40]!r}",
                )
        if html_count > 1:
        ...
        for el in self.state.head.elements():
            if el.tag not in ("meta", "title", "style", "script"):
                self.add("S029", ERROR, f"<{el.tag}> is not allowed in <head>", "head")
        for el in self.state.head.iter():
            if el is self.state.head:
                continue
            if el.tag in REGISTRY.forbidden_elements:
                self.add("X001", ERROR, f"<{el.tag}> is forbidden in .aim documents", "head")
            ...
            for card in sec.elements():
                ...
                for name, _ in card.attrs:
                    if name.startswith("on"):
                        self.add(
                            "X002", ERROR, f"event-handler attribute {name!r} is forbidden", where
                        )
                    elif name not in allowed and not name.startswith("data-x-"):
                        self.add(
                            "V003",
                            ERROR,
                            f"attribute {name!r} is not allowed on <aim-proposal>",
                            where,
                        )
        ...
        for el in self.state.body.elements():
            if el.tag == "aim-proposals":
                ...
                if stype == REGISTRY.script_types["history"]:
                    ...
                    self.add("S010", ERROR, f"unexpected script type {stype!r} in <body>")
                    continue
            else:
                rank = 0
                if el.chunk_id and el.container_id:
                    self.add(
                        "S012",
                        ERROR,
                        "an element cannot be both a chunk and a container",
                        el.chunk_id,
                    )
            ...
            if rank:
                ...
                if seen[rank] > 1:
                    self.add("S013", ERROR, f"more than one {el.tag or 'section'} of rank {rank}")
            if rank < last:
                self.add(
                    "S014",
                    ERROR,
    ...
    def chunks_and_runs(self) -> None:
            ...
            for el in parent.elements():
                ...
                if cid:
                    ...
                    if cid in seen_parent and seen_parent[cid] is not parent:
                        self.add(
                            "S016", ERROR, f"chunk {cid!r} appears under multiple parents", cid
                        )
                    elif cid in seen_parent and prev != cid:
                        self.add("S017", ERROR, f"run {cid!r} is not consecutive", cid)
                    seen_parent[cid] = parent
                if cont:
                    if not ids.is_valid_chunk_id(cont):
                        self.add("S015", ERROR, f"invalid container id {cont!r}", cont)
                    if cont in seen_container:
                        self.add("S018", ERROR, f"duplicate container id {cont!r}", cont)
        ...
        for d in dup:
            self.add("S019", ERROR, f"id {d!r} is used as both chunk and container id", d)
    ...
    def check_container(self, cont: Element) -> None:
        ...
        for child in cont.elements():
            ...
            if child.chunk_id:
                ...
                continue
            self.add("S023", ERROR, f"container {cid!r} child <{child.tag}> is uncovered", cid)
        ...
        if assets is not None:
            for el in assets.iter():
                ...
                if el.tag not in REGISTRY.asset_content:
                    self.add("V001", ERROR, f"<{el.tag}> is not allowed in the asset registry")
                else:
                    # the registry is not exempt from the security layer:
                    # attribute allowlists, on*, and URL schemes apply here too
                    self.check_element(el, context="assets", where=el.get("id") or "aim-assets")
        sec = self.state.section("aim-proposals")
        if sec is not None:
            for card in sec.elements():
    ...
    def check_element(self, el: Element, *, context: str, where: str = "") -> None:
        ...
        if tag in REGISTRY.forbidden_elements:
            ...
            return
        if tag in ("svg", "use", "image", "symbol", "rect", "circle", "ellipse", "path", "g"):
            pass  # svg subset shares the attr tables below
        elif tag not in REGISTRY.chunk_content and tag != "aim-slide":
            self.add(
                "V002",
                ERROR,
                f"<{tag}> is not in the v{REGISTRY.spec_version} element vocabulary",
                loc,
            )
            return
        ...
        for name, value in el.attrs:
            if name.startswith("on"):
                self.add("X002", ERROR, f"event-handler attribute {name!r} is forbidden", loc)
                continue
            ...
            if (
                name not in allowed
                and canonical_name not in allowed
                and not name.startswith("data-x-")
            ):
                self.add(
                    "V003", ERROR, f"attribute {canonical_name!r} is not allowed on <{tag}>", loc
                )
                continue
            if name in ("fill", "stroke") and value and "url(" in value:
                self.add(
                    "X003", ERROR, f"url() is not allowed in {tag}@{name}: {value[:40]!r}", loc
                )
                continue
    ...
    def check_style(self, value: str, loc: str) -> None:
        for piece in value.split(";"):
            ...
            if prop not in REGISTRY.style_prop_order:
                ...
                self.add(
                    "V008", ERROR, f"style value {val!r} does not match the {prop} grammar", loc
                )
    ...
    def check_url(self, tag: str, attr: str, value: str, loc: str) -> None:
        ...
        if low.startswith("javascript:") or low.startswith("data:text"):
            self.add("X003", ERROR, f"dangerous URL in {tag}@{attr}: {value[:40]!r}", loc)
            return
    ...
    def check_theme_block(self, raw: str, where: str) -> None:
        ...
        if not m:
            self.add("V010", ERROR, "theme block must be exactly one :root{…} rule", where)
            return
        for piece in m.group(1).split(";"):
            ...
            if not piece:
                continue
            if ":" not in piece:
                self.add("V010", ERROR, f"malformed theme declaration {piece!r}", where)
                continue
            name, value = (s.strip() for s in piece.split(":", 1))
            slot = REGISTRY.theme_slots.get(name)
            if slot is None:
                self.add("V011", ERROR, f"{name!r} is not a registered theme slot", where)
                continue
    ...
    def proposals(self) -> None:
                ...
                for child in node.elements():
                    if child.tag != "template":
                        self.add(
                            "P001",
                            ERROR,
                            f"unexpected <{child.tag}> inside a proposal card",
                            node.get("id") or "",
                        )
        ...
        for p in self.doc.proposals:
            ...
            if "data-for" in spec["requires"] and not p.target:
                self.add("P004", ERROR, f"{p.action} proposal needs data-for", where)
            if "data-anchor-container" in spec["requires"] and not p.anchor_container:
                self.add("P005", ERROR, f"{p.action} proposal needs data-anchor-container", where)
            if spec["payload"] and not p.payload_html:
                self.add(
                    "P006", ERROR, f"{p.action} proposal must carry a <template> payload", where
                )
            ...
            if p.action in ("modify", "delete") and p.target:
                if p.target in pending_md:
                    self.add(
                        ...
                        f"(already pending: {pending_md[p.target]})",
                        where,
                    )
            ...
            if p.action == "add" and p.anchor_after:
                ...
                if not ok:
                    ...
                    if anchor_prop is not None and (anchor_prop.anchor_container or "body") != (
                    ...
                    try:
                        self.state.resolve_insert_point(
                            Anchor(
                                p.anchor_container or "body", p.anchor_after, shell=p.anchor_shell
                            )
                        )
        ...
        try:
            for emb in self.doc.stale_embeddings():
                self.add(
                    "M002",
                    WARNING,
                    f"embedding for chunk {emb.get('chunk')!r} is stale or orphaned",
                    str(emb.get("chunk")),
                )
        except ParseError as exc:
    ...
    def canonical_form(self) -> None:
        ...
        for i, (a, b) in enumerate(zip(got, want, strict=False)):
            if a != b:
                ...
                return
        self.add(
            "C001", ERROR, f"file is not in canonical form (line count {len(got)} vs {len(want)})"
        )


# --------------------------------------------------------------------------
def lint(doc: AimDocument, *, source_text: str | None = None) -> list[Finding]:
    ...
    try:
        ...
        return [
            Finding(
                "S000",
                ERROR,
                f"verifier internal error (please report): {type(exc).__name__}: {exc}",
            )
        ]
