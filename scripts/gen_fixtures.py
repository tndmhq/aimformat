...
def base_doc() -> aim.AimDocument:
    doc = aim.new_document(title="Conformance fixture",
                           theme={"--aim-brand-1": "#1a73e8"})
    doc.add_chunk('<h1 data-aim="h1" class="font-bold text-3xl">Fixture</h1>',
                  author=BOT, at=t(0))
    doc.add_chunk('<p data-aim="p1">One paragraph &amp; some text.</p>',
                  author=BOT, at=t(1))
    doc.add_chunk('<ul data-aim-container="l1"><li data-aim="i1">First</li>'
...
def main() -> None:
    ...
    lifecycle = base_doc()
    p = lifecycle.propose_modify("p1", '<p data-aim="p1">Better text.</p>',
                                 author=BOT, explanation="Tighter.", at=t(3))
    lifecycle.accept(p.id, decided_by=ME, at=t(4))
    rej = lifecycle.propose_delete("i1", author=BOT,
                                   explanation="Redundant.", at=t(5))
    lifecycle.reject(rej.id, decided_by=ME, at=t(6))
    lifecycle.propose_add("<p>Pending addition.</p>", author=ME,
                          after="p1", explanation="More context.", at=t(7))
    lifecycle.checkpoint("reviewed", at=t(8))
    files["ok_lifecycle.aim"] = lifecycle.dumps()

    deck = aim.new_document(title="Deck fixture")
    deck.add_chunk('<aim-slide data-aim-container="s1" '
    ...
    for name in sorted(files):
        ...
        if name.startswith("nok_"):
            ...
            if want not in fired or errors != want_errors:
                print(f"  {name}: expected {want} (errors {want_errors}), "
