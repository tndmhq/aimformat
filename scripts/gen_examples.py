...
def proposal_doc() -> aim.AimDocument:
    ...
    doc = aim.new_document(
        title="Q3 Services Proposal — Acme GmbH", theme={"--aim-brand-1": "#1a73e8"}
    )
    with doc.batch():
        doc.add_chunk(
            '<h1 data-aim="ttl" class="font-bold text-3xl '
        ...
        doc.add_chunk(
            '<p data-aim="intro" class="text-gray-700 text-lg">'
            "We propose a three-year services engagement.</p>",
            author=BOT,
            at=t(1),
        )
        doc.add_chunk(
            '<section data-aim="scope"><h2 class="text-2xl">Scope'
        ...
        doc.add_chunk('<h2 data-aim="hd" class="text-2xl">Deliverables</h2>', author=BOT, at=t(3))
        doc.add_chunk(
            '<ul data-aim-container="dl">'
        ...
        doc.add_chunk(
            '<table data-aim-container="pr"><thead>'
    ...
    p = doc.propose_modify(
        "intro",
        '<p data-aim="intro" class="text-gray-700 '
    ...
    rej = doc.propose_delete(
        "d1", author=BOT, explanation="Audit is table stakes; cut it.", at=t(12)
    )
    doc.reject(rej.id, decided_by=ME, at=t(13))
    doc.checkpoint("sent-to-client", at=t(14))
    # still pending when the file is opened:
    doc.propose_add(
        '<tr data-aim="r3"><td>Rollout &amp; training</td><td>8</td><td>€95,000</td></tr>',
        author=BOT,
        container="pr",
        after="r2",
    ...
    doc.set_summary(
        "Three-year services proposal for Acme GmbH: €358k over "
