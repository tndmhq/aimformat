...
class TestAdd:
    ...
    def test_add_first_position_with_none(self, basic_doc):
        basic_doc.add_chunk('<p data-aim="new">Front.</p>', author=BOT,
                            after=None, at=ts(9))
        assert basic_doc.body_ids[0] == "new"

    def test_add_after_specific_chunk(self, basic_doc):
        basic_doc.add_chunk('<p data-aim="new">Mid.</p>', author=BOT,
                            after="h1", at=ts(9))
        assert basic_doc.body_ids[:3] == ["h1", "new", "intro"]

    def test_add_into_list_container(self, rich_doc):
        rich_doc.add_chunk('<li data-aim="li9">Ninth</li>', author=BOT,
                           container="list", after="li1", at=ts(9))
        html = rich_doc._state.serial("list")
        assert html.index('data-aim="li1"') < html.index('data-aim="li9"') \
            < html.index('data-aim="li2"')

    def test_add_first_into_list(self, rich_doc):
        rich_doc.add_chunk('<li data-aim="li0">Zeroth</li>', author=BOT,
                           container="list", after=None, at=ts(9))
        html = rich_doc._state.serial("list")
        assert html.index("li0") < html.index("li1")

    def test_add_row_into_table_shell(self, rich_doc):
        rich_doc.add_chunk('<tr data-aim="row3"><td>gamma</td><td>3</td></tr>',
                           author=BOT, container="tbl", after="row2", at=ts(9))
        assert '<tr data-aim="row3">' in rich_doc._state.serial("tbl")

    def test_add_run_of_items(self, rich_doc):
        c = rich_doc.add_chunk('<li data-aim="run">a…</li>'
                               '<li data-aim="run">…b</li>', author=BOT,
                               container="list", at=ts(9))
        assert c.is_run and c.tags == ("li", "li")

    def test_add_run_outside_container_rejected(self, basic_doc):
        with pytest.raises(InvalidOperation):
            basic_doc.add_chunk('<p data-aim="r">a</p><p data-aim="r">b</p>',
                                author=BOT, at=ts(9))

    def test_add_into_unknown_container_raises(self, basic_doc):
        with pytest.raises(TargetNotFound):
            basic_doc.add_chunk("<li>x</li>", author=BOT, container="nope",
                                at=ts(9))
    ...
    def test_add_records_event_with_anchor_and_payload(self, basic_doc):
        basic_doc.add_chunk('<p data-aim="new">x</p>', author=BOT,
                            after=None, at=ts(9))
    ...
    def test_add_slide_container_whole_subtree(self, basic_doc):
        basic_doc.add_chunk('<aim-slide data-aim-container="s9" '
                            'style="width:1920px; height:1080px">'
                            '<h2 data-aim="t9" style="left:1px; top:2px">T</h2>'
                            "</aim-slide>", author=BOT, at=ts(9))
        ev = basic_doc.history[-1]
...
class TestModify:
    def test_modify_replaces_and_logs_before_after(self, basic_doc):
        ...
        basic_doc.modify_chunk("intro", '<p data-aim="intro">Better.</p>',
                               author=ME, at=ts(9))
        ev = basic_doc.history[-1]
    ...
    def test_modify_payload_without_id_gets_target_id(self, basic_doc):
        basic_doc.modify_chunk("intro", "<p>No id, adopted.</p>", author=ME,
                               at=ts(9))
...
class TestDeleteMove:
    ...
    def test_move_between_containers(self, rich_doc):
        rich_doc.add_chunk('<ul data-aim-container="list2">'
                           '<li data-aim="lx">x</li></ul>', author=BOT, at=ts(8))
        rich_doc.move_chunk("li1", container="list2", after="lx",
                            author=ME, at=ts(9))
    ...
    def test_move_run_moves_all_members(self, rich_doc):
        rich_doc.move_chunk("li2", container="list", after=None,
                            author=ME, at=ts(9))
        html = rich_doc._state.serial("list")
    ...
    def test_container_move_in_body(self, rich_doc):
        rich_doc.move_chunk("list", container="body", after=None,
                            author=ME, at=ts(9))
