...
def basic_doc() -> aim.AimDocument:
    ...
    doc = aim.new_document(title="Basic fixture", theme={"--aim-brand-1": "#1a73e8"})
    doc.add_chunk('<h1 data-aim="h1" class="font-bold text-3xl">Title</h1>', author=BOT, at=ts(0))
    doc.add_chunk('<p data-aim="intro">Intro paragraph.</p>', author=BOT, at=ts(1))
    return doc
...
def rich_doc() -> aim.AimDocument:
    ...
    doc = aim.new_document(title="Rich fixture", theme={"--aim-brand-1": "#1a73e8"})
    with doc.batch():
        doc.add_chunk(
            '<h1 data-aim="h1" class="font-bold text-3xl">Report</h1>', author=BOT, at=ts(0)
        )
        doc.add_chunk('<p data-aim="intro">We looked at the numbers.</p>', author=BOT, at=ts(1))
        doc.add_chunk(
            '<section data-aim="scope"><h2>Scope</h2><p>Everything shipped in Q2.</p></section>',
            author=BOT,
            at=ts(2),
        )
        doc.add_chunk(
            '<ul data-aim-container="list"><li data-aim="li1">First'
...
def lifecycle_doc(rich_doc: aim.AimDocument) -> aim.AimDocument:
    ...
    p2 = doc.propose_modify(
        "li1",
        '<li data-aim="li1">First (reworded)</li>',
        author=BOT,
        explanation="Reword.",
        at=ts(12),
    )
    ...
    doc.modify_chunk(
        "row1", '<tr data-aim="row1"><td>alpha</td><td>11</td></tr>', author=ME, at=ts(16)
    )
